    ...
    ui(new Ui::MainWindow)
{
    ...
    QLabel* cLabel = new QLabel("Kymograph view", this);
    aLabel->setGeometry(50,80,100,20);
    bLabel->setGeometry(50,280,100,20);
    cLabel->setGeometry(50,380,100,20);
    ...
    QLabel* stepsLabel = new QLabel("   Step(s) per update:", this);
...
{
    QMessageBox::about(this, "About Application", "Simulation of bidirectional transport of two motor species on a MT bundle.<br><br>Code by Congping Lin and Peter Ashwin<br>In collaboration with Gero Steinberg and Martin Schuster<br><br>University of Exeter, 2018.");
}
        ...
        if (flag){
            ...
            if (tr<worker->bundle->trackNum){
                ...
                if (length>0 && width>0 && para.distance.size()==para.dynein.size() && para.distance.size()==para.plus.size()){
                    ...
                    QMessageBox msgBox;
                    QString s("Length and width of a track should be a positive integer!");
                    msgBox.setText(s);
                    msgBox.exec();
                }
                ...
                QMessageBox msgBox;
                QString s("Track index should be non-zero integer and no more than ");
                s.append(QString::number(Nmax));
                msgBox.setText(s);
                msgBox.exec();
            }
...
void MainWindow::checkBundleAccess(){
    ...
    if (k<maxMT){
        QMessageBox msgBox;
        QString s("Bundle organization is valid!");
        msgBox.setText(s);
        msgBox.exec();
    }
}
...
void MainWindow::createActions(){
    ...
    kymographSpeedAct=new QAction(tr("&Set Kymograph Speed"),this);
...
void MainWindow::creatDir(){
    dirname = QInputDialog::getText(this, "Enter New Folder Name", "Enter name");
    QDir temp;
    temp.mkdir(dirname);
}
...
void MainWindow::importBundle(){
    ...
    // uncomment below to use Qstring to import bundle
        ...
        while(tr<temp_bundle->trackNum){
            ...
            Track temp(len,list2[0].toInt());//temporary track with length L and width list2[0].toInt()
            ...
            for (int la=0;la<temp_bundle->getTrack(tr)->width;la++){
                ...
                for (int x=0;x<len;x++){
                    ...
                    if (occu0==1 && occu1==0){
                        ...
                        QMessageBox msgBox;
                        QString s("Not satisfying exclusion principle at site !");
...
void MainWindow::lotsUpdate()
{
    ...
    for (int i=0;i<n;i++){
        statusBar()->showMessage("Updating every "+QString::number(n)+" step(s). "+" Time elapsed "+QString::number(t)+" s");
        worker->oneStep();
        ...
        if (tStep==1){
            ...
            for (int j=0;j<tStep-worker->previousStep;j++){
                for (int i=0;i<static_cast<int>(worker->bundle->vkymograph[0].size());i++){
                    if (worker->bundle->vkymograph[j][i]>0)
                        previousKymographImage.setPixel(i,tStep-j,qRgb(255.*(worker->bundle->vkymograph[j][i]%2==1),255.*(worker->bundle->vkymograph[j][i]>1),255.*0));
                    else  previousKymographImage.setPixel(i,tStep-j,qRgb(255,255,255));
                }
            }
            worker->previousStep=tStep;
        }
            ...
            for (int i=0;i<previousKymographImage.width();i++){
                for (int j=0;j<previousKymographImage.height();j++){
                    // the pixel value can be 0(empty),1(kinesin only),2(dynein only),3(both)
                    if (worker->bundle->vkymograph[j][i]>0)
                        previousKymographImage.setPixel(i,tStep-j,qRgb(255.*(worker->bundle->vkymograph[j][i]%2==1),255.*(worker->bundle->vkymograph[j][i]==2),255.*(worker->bundle->vkymograph[j][i]>2)));
                }
            }
            worker->previousStep=tStep;
        }
            ...
            for (int j=0;j<tStep-worker->previousStep;j++){
                for (int i=0;i<mx;i++){
                    ...
                    if (c>0){
                        ...
                        newupdate.setPixel(i,tStep-j,qRgb(255.*(c%2==1),255.*((c%2==0)+(c%3==0)),150.*(c>=10)));
                    }
                }
            }
            ...
            for (int i=0;i<previousKymographImage.width();i++){
                for (int j=0;j<previousKymographImage.height();j++){
                    ...
                    if (c>0){
                        ...
                        previousKymographImage.setPixel(i,tStep-j,qRgb(255.*(c%2==1),255.*((c%2==0)+(c%3==0)),150.*(c>=10)));
                    }
                }
            }
            worker->previousStep=tStep;
...
void MainWindow::showKymographTimeDown(){
    ...
    if(show){
        ...
        if (tStep==1){
            ...
            int ttStep=tStep-worker->previousStep-1;
            for (int j=ttStep;j>=0;j--){
                for (int i=0;i<mx;i++){
                    int c=worker->bundle->vkymograph[j][i];
                    if (c>0){
                        // int rc=255*(c>0);//255.*(worker->bundle->vkymograph[j][i]%2==1)
                        //int gc=255*(c>0);//255.*(worker->bundle->vkymograph[j][i]>1)
                        newupdate.setPixel(i,ttStep-j,qRgb(255.*(c>0),255.*(c>0),255.*(c>0))); // for white signals
...
void MainWindow::showTipSize(){
    ...
    if (mx>0){
        ...
        plotWindow->customPlot->xAxis->setLabel(" step(s)");
...
void MainWindow::startRun()
{
    ...
    while(worker->running==true){
        for (int i=0;i<n;i++){
            QString s("Updating every ");
            s.append(QString::number(n)+" step(s). ");
            s.append("Time elapsed "+QString::number(t)+" s");
